...
def deneme_menu():
        pop_up=Menu(mbtn.menu,tearoff=0)
        mbtn["menu"]=pop_up
        pop_up.add_command(label="Dönem Sonu Sonsuz Artan Ödemeler Serisi",command=Donem_sonu_sonsuz_Artan_odemeler_Serisi)
        pop_up.add_command(label="Çıkış",command=quit) # geçiş yapıyor fakat tekrar menüyü açmak zorunda klaıyoruz.     
#menü 2. sayfa Olabilir bu kısım
mbtn.menu.add_command(label="Dönem Sonu Sonsuz Artan Ödemeler Serisi",command=Donem_sonu_sonsuz_Artan_odemeler_Serisi)
mbtn.menu.add_command(label="Dönem Başı Sonsuz Artan Ödemeler Serisi",command=Donem_basi_sonsuz_artan_ödemler_serisi)
mbtn.menu.add_command(label="Sürekli Ödemeli Artan Ödemeler Serisi",command=Surekli_ödemeli_artan_ödemeler_Serisi)
mbtn.menu.add_command(label="Sürekli Ödemeli Azalan Ödemler Serisi",command=Surekli_ödemeli_azalan_ödemeler_serisi)
mbtn.menu.add_command(label="Sürekli Ödemeli Artan Sonsuz Ödemeler Serisi",command=Surekli_ödemeli_artan_sonsuz_ödemeler_serisi)
mbtn.menu.add_command(label="Üssel Artan Dönem Sonu Ödemeler Serisi",command=Ussel_artan_dönem_sonu_ödemeler_serisi)
mbtn.menu.add_command(label="Üssel Artan Dönem Başı Ödemeler Serisi",command=Ussel_artan_dönem_basi_ödemeler_serisi)
mbtn.menu.add_command(label="Sürekli Değişen Nakit Akışı",command=Surekli_degisken_nakit_akisi)
mbtn.menu.add_command(label="T Anında t Ödeme Oranlı Sürekli Artan Seri",command=T_anında_t_ödeme_oranlı_surekli_artan_seri)
mbtn.menu.add_command(label="T Anında n-t Ödeme Oranlı Sürekli Azalan Seri",command=T_anında_n_t_ödeme_oranlı_sürekli_azalan_seri)
mbtn.menu.add_command(label="T Anında t Ödeme Oranlı Sürekli Sonsuz Artan Seri",command=T_anında_t_ödeme_oranlı_surekli_sonsuz_artan_seri)
mbtn.menu.add_command(label="Net Peşin Değer",command=Net_pesin_deger)
mbtn.menu.add_command(label="İç Verim/Getiri Oranı Karşılaştırma",command=iç_verim_getiri_oranı_karsılastırma)
mbtn.menu.add_command(label="Nominal ve Gerçek Nakit Akışlarının Peşin Değeri",command=nominal_ve_gercek_nakit_akıslarının_pesin_degeri)
mbtn.menu.add_command(label="Para Ağırlıklı Faiz/Getiri Oranı",command=para_agirlikli_faiz_getiri_orani)
mbtn.menu.add_command(label="Zaman Ağırlıklı Faiz/Getiri Oranı")
#menü 3. sayfa eklenebilir.

#Ayırıcı çizgi
mbtn.menu.add_separator()

mbtn.menu.add_command(label="Devam >>",command=deneme_menu)# menu içerisinde 2. sayfa ekleme yöntemi bakılacak.
